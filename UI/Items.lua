...
--
-- Ячеек три (см. SB.Items.GetMaxPrepared). Колонка предметов ровно такой же
-- ширины, что и колонка активных эффектов, и делает она это нарочно —
-- обе стоят рядом и обязаны совпадать по краю.
--
-- Поэтому COLS ТОТ ЖЕ, ЧТО У ЭФФЕКТОВ: три в ряд. Сетка осталась
-- сеткой, хотя потолок и опустился до трёх: раскладка считается от
-- GetMaxPrepared, и вернись потолок к шести — второй ряд появится сам,
-- без правок здесь.
-- ============================================================
local COLS = 3
...
---
--- Считается от числа ячеек, а не константой: вернись их больше трёх —
--- второй ряд займёт место сам (см. SB.UI.UpdateItemSlots).
...
function SB.UI.BuildItemsColumn(body)
    ...
    -- ЗАВОДИМ СРАЗУ ПО ПОТОЛКУ, а показываем открытые.
    for i = 1, SB.Items.MAX_PREPARED do
