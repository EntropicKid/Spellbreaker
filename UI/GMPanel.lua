...
function SB.UI.UpdateGMPlayers()
    ...
    for index, p in ipairs(allPlayers) do
        ...
        if not row then
            ...
            --
            -- ПО ЦЕНТРУ ПОРТРЕТА — так же, как на рамках юнитов. Сначала
            -- значок стоял в углу, чтобы не закрывать лицо, но это
            -- развело две отметки одного и того же: на рамке значок в
            -- центре и крупный, в панели — сбоку и мелкий, и глаз искал
            -- его в разных местах. Одно и то же обязано читаться
            -- одинаково.
            --
            -- ТЕКСТУРА ЖИВЁТ НА САМОМ ПОРТРЕТЕ, а не на строке. Портрет —
            -- ОТДЕЛЬНАЯ РАМКА внутри строки (см. SB.Theme.RoundPortrait), а
            -- дочерняя рамка рисуется поверх ВСЕХ слоёв родителя, включая
            -- OVERLAY: значок на строке уезжал под кольцо. Внутри портрета
            -- порядок решает слой, и OVERLAY выше и картинки
            -- (BACKGROUND), и кольца (ARTWORK), и иконки класса.
            --
            -- ПОДСЛОЙ ЯВНО: внутри одного слоя порядок решает подслой, а
            -- не то, что создано позже.
            row.turnMark = row.portrait:CreateTexture(nil, "OVERLAY")
            row.turnMark:SetDrawLayer("OVERLAY", 7)
            -- Две трети портрета — столько же, сколько значок занимает на
            -- портрете своей рамки (34 из 52): там его и привыкли видеть.
            row.turnMark:SetSize(34, 34)
            row.turnMark:SetPoint("CENTER", row.portrait, "CENTER", 0, 0)
            row.turnMark:Hide()
