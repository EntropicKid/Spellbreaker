...
SB.Data.Attributes = {
    { key = "Сила",
	  ...
	  skills    = { "Ношение брони", "Запугивание", "Мощь", "Искусность" } },
    ...
    { key = "Характер",
	  ...
	  skills    = { "Внушение", "Воодушевление", "Милосердие", "Рвение" } },
}

-- База — общая с навыками и существами (см. SB.Data.STAT_BASE): одно и
-- то же число обязано значить одно и то же везде.
local MIN_ATTR           = SB.Data.STAT_BASE or 0
local MAX_ATTR           = 5
...
for _, def in ipairs(SB.Data.Attributes) do
    SB.Data.Tooltips["attr_" .. def.key] = {
        ...
        lines = {
            ...
            -- ехать вместе с ними.
            ("Каждое вложенное очко даёт +%d к проверкам этой характеристики и к заклинаниям, которые от неё скейлятся.")
...
--- 3 базовых + 1 каждые два уровня — на 3-м, 5-м, 7-м и дальше по
--- нечётным: 8 к 11-му, 13 к 21-му, 15 на капе.
---
--- ШАГ В ДВА УРОВНЯ, А НЕ В ТРИ, и это следствие базы в ноль, а не
--- отдельная щедрость. С базой в единицу персонаж рождался с шестью
--- бесплатными очками — по одному в каждом атрибуте, — просто они ничего
--- не давали. Теперь атрибуты начинаются с нуля, и то, что было фоном,
--- приходится добирать вложением: частые очки возвращают в лист то, что
--- из него ушло вместе с бесплатной единицей.
---
--- ОТСЧЁТ ОТ ПЕРВОГО УРОВНЯ: (уровень − 1) / 2, а не уровень / 2. Иначе
--- очко приходило бы на чётных уровнях, а обещано — на третьем, пятом,
--- седьмом.
---
...
-- Очки на старте. Отдельной ручкой, а не числом в формуле: это то, что
-- двигают при балансировке, и оно уже успело побыть пятёркой, двойкой
-- и тройкой.
local ATTR_POINTS_START = 3
...
function SB.Attributes.GetTotalPoints(level)
    ...
    return ATTR_POINTS_START + math.floor(math.max(0, ref - 1) / 2)
