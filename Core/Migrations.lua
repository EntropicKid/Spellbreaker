...
-- МИГРАЦИЯ ВИДИТ ТОЛЬКО ТО, ЧТО РЕАЛЬНО ЛЕЖИТ В ФАЙЛЕ. Значения,
-- совпадающие с умолчанием AceDB, из файла вырезаны и приходят
-- метатаблицей: pairs() их не перечислит, rawget вернёт nil. Обходя
-- таблицу с умолчаниями, считай отсутствующий ключ равным умолчанию
-- ТОЙ версии, в которой данные записаны (см. врезку в Core/Init.lua и
-- v14 ниже — так v12 и пропустила атрибуты на единице).
SB.SCHEMA_VERSION = 14
...
SB.Migrations.List = {
    ...
    {
        version = 9,
        note = "«Дипломатия» стала «Воодушевлением»",

        -- Навык переименован вместе с реворком: у него появилась
        -- механика (срок баффов на союзниках), и старое имя её больше не
        -- описывало (см. SB.Skills.GetEncouragementBonus).
        --
        -- БЕЗ ЭТОГО ШАГА ВЛОЖЕННЫЕ ОЧКИ ПРОСТО ПРОПАЛИ БЫ. Навыки лежат в
        -- сохранёнке по ИМЕНИ-КЛЮЧУ; никакого id у них нет, и переезд имени
        -- для базы неотличим от «старый навык удалили, новый добавили» —
        -- причём молча: незнакомый ключ просто никем не читается.
        --
        -- ЧЕРНОВИК НЕ ТРОГАЕМ: он живёт в памяти до подтверждения и до
        -- сохранёнки не доходит (см. Core/Skills.lua).
        char = function(db, report)
            local skills = db.skills
            if type(skills) ~= "table" then return end

            local old = skills["Дипломатия"]
            if old == nil then return end
            skills["Дипломатия"] = nil

            -- Если по обоим ключам что-то лежит (сохранёнка пережила
            -- полуручную правку), берём БОЛЬШЕЕ: отнять вложенное молча
            -- хуже, чем оставить лишнее — второе игрок хотя бы увидит.
            local new = tonumber(skills["Воодушевление"]) or 0
            skills["Воодушевление"] = math.max(new, tonumber(old) or 0)

            report("навык «Дипломатия» переименован в «Воодушевление», " ..
                   "вложенное сохранено: " .. skills["Воодушевление"] .. ".")
        end,
    },

    {
        version = 10,
        note = "Короткий Отдых упразднён",

        -- Механики больше нет — ни групповой, ни личной. Поле зарядов
        -- осталось бы в сохранёнке навсегда: AceDB не убирает то, чего
        -- никто не читает.
        --
        -- ЧИСТИМ, А НЕ ОСТАВЛЯЕМ. Мёртвое поле в базе живёт тихо ровно до
        -- дня, когда кто-нибудь заведёт рядом поле с похожим смыслом и
        -- прочтёт это как своё. Ровно поэтому же с этой правкой ушли
        -- рычаги restHeal и restCharges из профилей рас и классов.
        --
        -- СТРОКИ В ЧАТ НЕТ НАМЕРЕННО: игрок ничего не терял. Отдых ему
        -- заменил ручеёк от «Лидерства», а сообщать о вычищенном
        -- служебном поле — значит тревожить по поводу, которого нет.
        char = function(db, report)
    ...
    {
        version = 12,
        note = "атрибуты и навыки отсчитываются от нуля",

        -- База характеристик сменилась с единицы на ноль
        -- (см. SB.Data.STAT_BASE). Сохранённое значение — это число из
        -- листа, а не «сколько вложено», поэтому без перевода каждая
        -- характеристика персонажа разом прибавила бы по очку: «Сила 3»
        -- была двумя вложенными очками, а по новому счёту стала бы тремя.
        -- Лист молча вырос бы на шесть очков атрибутов и на все навыки, а
        -- распределённое оказалось бы больше выданного.
        --
        -- СДВИГ НА ЕДИНИЦУ СОХРАНЯЕТ ВСЁ РАЗОМ: и сколько вложено, и что
        -- это даёт. Прибавка считается от базы, база сдвинулась на
        -- единицу, значение сдвигается на неё же — разница не меняется ни
        -- у одной характеристики. Персонаж после перевода бросает и
        -- получает ровно то же, что до него; меняется только то, какие
        -- числа ему показывают.
        --
        -- ЕДИНИЦА ЗДЕСЬ ГОЛАЯ, И ТАК И НАДО: это база, в которой ДАННЫЕ
        -- БЫЛИ ЗАПИСАНЫ, а не текущая. Читай миграция SB.Data.STAT_BASE,
        -- она сдвигала бы на ноль.
        --
        -- ПО МЕТКЕ, А НЕ ПО НОМЕРУ ВЕРСИИ. Миграции обязаны переживать
        -- повторный прогон после ручного отката версии (см. врезку над
        -- списком), а сдвиг идемпотентным не бывает по природе: прогони
        -- его дважды — снимет два. Метка statsBase говорит, в какой базе
        -- лежат числа, и второй прогон уходит, ничего не тронув.
        char = function(db, report)
            if tonumber(db.statsBase) == 0 then return end
            ...
            if shifted > 0 then
                report("характеристики теперь отсчитываются от нуля: каждое " ..
                       "число в листе стало на единицу меньше, но вложенные " ..
                       "очки и всё, что они дают, остались прежними.")
            end
        end,
    },
    ...
    {
        version = 13,
        note = "«Ремесло» стало «Искусностью»",

        -- Имя навыка — ключ в сохранёнке, и очки под старым ключом никто
        -- бы больше не прочёл: персонаж молча потерял бы вложенное. Сама
        -- таблица имён — SB.Data.SkillRenames; здесь только персонаж
        -- (существа и свои заклинания переводятся там, где их читают).
        --
        -- Идемпотентна без метки: второй прогон старых ключей не найдёт.
        char = function(db, report)
            local moved = SB.Data.RenameSkillKeys(db.skills)
            if moved > 0 then
                report("навык «Ремесло» теперь зовётся «Искусность» — " ..
                       "вложенные очки на месте.")
            end
        end,
    },
    ...
    {
        version = 14,
        note = "фантомные единицы атрибутов",

        -- До этой версии у атрибутов было значение по умолчанию — единица
        -- (см. CHAR_DEFAULTS в Core/Init.lua). AceDB не пишет в файл то,
        -- что совпадает с умолчанием, поэтому ОТСУТСТВУЮЩИЙ атрибут в
        -- сохранёнке означал «единица». При базе ноль это фантомное очко:
        -- оно появлялось у каждого невложенного атрибута после сброса и
        -- у невложенных атрибутов старых персонажей (миграция v12 их не
        -- видела — их не было в файле).
        --
        -- Умолчания больше нет, и отсутствующий атрибут читается нулём.
        -- Честно вложенную РОВНО ОДНУ единицу AceDB вырезал точно так же,
        -- и отличить её от фантомной по файлу нельзя. Ошибаемся в
        -- безопасную сторону: очко не пропадает, а возвращается в
        -- свободные, и игроку об этом сказано.
        --
        -- Писать ничего не нужно — только сказать. Пустая таблица —
        -- новый персонаж: ему нечего возвращать.
        char = function(db, report)
            local attrs = db.attributes
            if type(attrs) ~= "table" or next(attrs) == nil then return end
            local back = {}
            for _, def in ipairs(SB.Data.Attributes or {}) do
                if rawget(attrs, def.key) == nil then back[#back + 1] = def.key end
            end
            if #back > 0 then
                report("атрибуты без вложений теперь честно равны нулю: " ..
                       table.concat(back, ", ") .. ". Если в каком-то из них " ..
                       "было ровно одно очко, оно вернулось в свободные — " ..
                       "вложите его заново.")
            end
        end,
    },
