...
local CHAR_DEFAULTS = {
    ...
    -- Прежний журнал одной строкой. Теперь журнал — записи в logBook
    -- (см. Core/LogStore.lua); старая строка переносится туда при первом
    -- входе и обнуляется.
    logHistory     = "",
...
initFrame:SetScript("OnEvent", function(self, event, loadedAddon)
    ...
    SB:RegisterChatCommand("sb", function(input)
        ...
        -- «/sb log [текст]» — журнал; с текстом — сразу с поиском по нему.
        -- Текст берётся из исходной строки, а не из разобранной: разбор
        -- выше приводит всё к нижнему регистру и берёт одно слово.
        if cmd == "log" or cmd == "logs" then
            if SB.Logs and SB.Logs.Open then
                local text = (input or ""):match("^%s*%S+%s+(.-)%s*$")
                SB.Logs.Open(text or nil)
            end
            return
        end
