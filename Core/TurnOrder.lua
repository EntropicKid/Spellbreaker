...
--
-- КЛЮЧОМ СЛУЖИТ САМ ХОД (сцена + круг + слот), а не флаг со сбросом.
-- Флаг пришлось бы обнулять во всех местах, где очередь двигается, и
-- однажды одно из них забылось бы: игрок получил бы второе бонусное
-- действие ровно в том круге, где его быть не должно. Ключ не
-- забывается — он меняется сам вместе с ходом.
--
-- НОМЕР СЦЕНЫ В КЛЮЧЕ ОБЯЗАТЕЛЕН, и его там сначала не было. «Круг плюс
-- слот» при перезапуске пошагового режима даёт РОВНО ТО ЖЕ значение —
-- «1:1», — то есть первый ход новой сцены выглядел для бонусного
-- действия продолжением первого хода прошлой. Наружу это было видно
-- так: после Долгого Отдыха и нового включения режима первое же зелье
-- съедало полный ход вместо бонусного, и только оно одно — со второго
-- круга всё работало. Номер сцены растёт на каждом TO.Start и едет в
-- том же пакете, что круг со слотом, так что новых полей это не стоит.
--
...
local function TurnKey()
    ...
    return (state.session or 0) .. ":" ..
           (state.round   or 0) .. ":" .. (state.index or 0)
end
...
-- ============================================================
-- ОТМЕТКА УЧАСТНИКА: ОДНО ПРАВИЛО НА ВСЕ РАМКИ
--
-- Отметку рисуют ДВА разных места — рамки юнитов (UI/Overlay.lua) и
-- список игроков в панели Ведущего (UI/GMPanel.lua), — и решать, какую
-- именно, они обязаны одинаково. Пока правило жило в одном из них,
-- второе либо не показывало ничего, либо однажды разошлось бы с первым:
-- «у Ведущего галочка, на рамке крестик» — это хуже, чем не показывать
-- вовсе.
--
-- ПОРЯДОК ПРОВЕРОК — ЭТО ПРИОРИТЕТ. «Отыграл» и «пропустил» — ИТОГ
-- хода, «ждём» — его отсутствие. Спроси мы сначала про ожидание, в
-- режиме «все сразу» галочка отыгравшего мигала бы вопросом до конца
-- круга.
-- ============================================================

--- Что показать по этому имени: "acted" | "skipped" | "waiting" | nil.
--- @param current table|nil  готовый набор имён, чей ход идёт сейчас
---        (см. TO.CurrentNameSet). Нужен там, где отметку спрашивают на
---        каждую из девяти десятков рамок десять раз в секунду: строить
---        набор заново на каждую рамку — девятьсот таблиц в секунду на
...
--- Набор имён, чей ход идёт прямо сейчас: { ["Имя"] = true }.
--- Разбирает оба режима и читает состав очереди, поэтому «ждём» не
--- достанется тому, кого в очереди нет вовсе (см. TO.GetCurrentNames).
function TO.CurrentNameSet()
    local set = {}
    for _, n in ipairs(TO.GetCurrentNames()) do set[n] = true end
    return set
end
