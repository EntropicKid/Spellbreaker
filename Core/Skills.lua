...
--- На сколько ходов дольше держится бафф, который ты кладёшь на другого.
---
--- Считается У ЗАКЛИНАТЕЛЯ и едет вместе с эффектом: длительность
--- собирает получатель (см. SB.Logic.GetEffectDuration), а про чужой
--- навык он ничего не знает — ровно та же развилка, по которой
...
SB.Data.SkillEffects = {
    ...
    ["Воодушевление"]  = "Каждое очко сверх 1 добавляет ОДИН ХОД сроку баффа, " ..
                         "который ты кладёшь на СОЮЗНИКА. На себя не работает: " ..
                         "навык покупает не собственную крепость, а способность " ..
                         "держать отряд. Зеркало «Воли» — та режет срок входящего " ..
                         "дебаффа, эта продлевает исходящий бафф.",
