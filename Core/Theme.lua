...
-- ── ОТМЕТКИ ХОДА ─────────────────────────────────────────────
--
-- Картинки из ПРОВЕРКИ ГОТОВНОСТИ, и это не экономия на рисовании:
-- игрок читает их не задумываясь, потому что видел сотни раз и ровно в
-- этих смыслах — «готов», «не готов», «от тебя ждут ответа». Свои
-- потребовали бы объяснения там, где объяснять нечего.
--
-- Здесь, а не в UI: их читают два разных места (рамки юнитов и список
-- игроков в панели Ведущего), а расходиться отметкам одного и того же
-- состояния нельзя (см. TO.MarkFor).
SB.Theme.TURN_MARK = {
    acted   = "Interface\\RaidFrame\\ReadyCheck-Ready",
    skipped = "Interface\\RaidFrame\\ReadyCheck-NotReady",
