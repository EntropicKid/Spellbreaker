...
--
-- Кожа (Tome.tga) — на библиотеку, и только на неё: переплёт книги
-- заклинаний (см. SURFACES.library).
--
-- ПРО РАЗМЕР. Сторона обязана быть степенью двойки — клиент не грузит
-- прочее вовсе, молча, без ошибки в логе (проверяется прогоном для всех
-- текстур аддона, см. Tests/run.lua).
SB.Theme.Assets.Wood    = MEDIA .. "Wood.tga"
-- Чёрный мрамор с золотыми жилами — колонки главного окна (SURFACES.column).
...
-- ============================================================
-- ПЛИТКА ПОДЛОЖКИ — В ТАКТ С ВЫСОТОЙ
--
-- Шаблон подложки пересчитывает координаты плитки по событию смены
-- размера, а оно приходит КАДРОМ ПОЗЖЕ. Пока высоту анимируют, каждый
-- кадр текстура рисуется со старыми координатами на новой высоте — то
-- есть чуть растянута или сжата по вертикали. Зовётся сразу после
-- SetHeight в анимированной раскладке.
-- ============================================================
function SB.Theme.SyncBackdrop(frame)
    if frame and frame.OnBackdropSizeChanged then
        pcall(frame.OnBackdropSizeChanged, frame)
    end
end
