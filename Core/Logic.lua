...
function SB.Logic.GetEffectDuration(effectID, sourceSpell, slotLevel, extraTurns)
    ...
    --
    -- ОКРУГЛЕНИЕ ВВЕРХ. Пятая часть однoходового бафа — это 0,2, и вниз
    -- она превращает навык в «ничего» ровно там, где срок и есть всё
    -- содержание заклинания. Вверх — значит любое вложение даёт хотя бы
    -- ход. Следствие честное и его надо знать: одноходовому баффу и
    -- одно очко, и пять дают один и тот же ход. Дробить нечего.
    local share = tonumber(SB.Data.Config and SB.Data.Config.EncouragementPerPoint) or 0.2
    local extra = (pts > 0) and math.ceil(turns * share * pts) or 0
