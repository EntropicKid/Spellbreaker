...
function SB.Logic.GetSpellRepair(spell, slotLevel)
    ...
    local scaled = SB.Logic.GetSpellScaling(spell, "armor")
...
function SB.Logic.GetEffectDuration(effectID, sourceSpell, slotLevel, extraTurns)
    ...
    -- ПОТОЛОК — СКОЛЬКО ОЧКОВ ВООБЩЕ БЫВАЕТ У НАВЫКА, то есть максимум
    -- атрибута за вычетом невложенного минимума. Минус единица стоял
    -- здесь с тех пор, когда база характеристик была единицей; после
    -- перехода на нулевую базу он молча съедал ПЯТОЕ очко навыка —
    -- полностью вложенное «Воодушевление» зажималось до четырёх и
    -- последнее вложение не давало ничего.
    local maxAttr = (SB.Attributes and SB.Attributes.GetMaxValue
        and SB.Attributes.GetMaxValue()) or 5
    local cap = maxAttr - (tonumber(SB.Data.STAT_BASE) or 0)
    local pts = math.max(0, math.min(cap, math.floor(tonumber(extraTurns) or 0)))

    -- ── ВООДУШЕВЛЕНИЕ — ДОЛЯ, А НЕ ХОДЫ ─────────────────────
    --
    -- Прибавка была плоской: очко = ход. По библиотеке это значило
    -- бессмыслицу на обоих концах. Из 196 баффов 80 держатся сто ходов
    -- и дольше или бессрочны — там пять ходов не видно вовсе; а 60
    -- держатся один-три хода, и те же пять ходов раздували их на
    -- 170-500%. Один навык одновременно не делал ничего половине
    -- библиотеки и переписывал личность трети: «Длань защиты» на один
    -- ход становилась шестиходовой.
    --
    -- Теперь доля: каждое очко продлевает на Config.EncouragementPerPoint
    -- от БАЗОВОГО срока, полностью вложенный навык — вдвое. Правило
    -- держится в голове одной фразой, а перекос уходит сам: у длинного
    -- бафа доля велика в ходах и незаметна по сути, у короткого —
    -- наоборот, и оба остаются собой.
    --
    ...
    -- ── ОКРУГЛЕНИЕ ВНИЗ ─────────────────────────────────────
    --
    -- Вверх — значит ЛЮБОЕ вложение даёт хотя бы ход, и на коротких
    -- сроках это не прибавка, а подарок: пятая часть одноходового
    -- заклинания — 0,2, вверх это целый ход, то есть ОДНО очко
    -- удваивает «Рывок». Живая жалоба ровно такая. Хуже того, дальше
    -- навык замирал: и одно очко, и пять давали один и тот же ход, и
    -- четыре вложенных очка не значили ничего.
    --
    -- Вниз — доля считается честно. Одноходовому нужны все пять очков,
    -- чтобы стать двухходовым; двухходовому хватит трёх.
    --
    -- РАЗЛИЧИМЫХ СТУПЕНЕЙ ПРИ ЭТОМ СТАЛО БОЛЬШЕ, а не меньше. У срока в
    -- три хода — самого частого в библиотеке — вверх выходило
    -- 4/5/5/6/6: три разных числа на пять очков, и второе с четвёртым
    -- не значили ничего. Вниз выходит 3/4/4/5/6 — четыре. Округление
    -- отбирает у первого очка, но возвращает смысл четвёртому.
    --
    -- ОБЕЩАНИЕ НАВЫКА ЦЕЛО. «Полностью вложенный — вдвое» держится
    -- ТОЧНО и на любом сроке: доля пять на пять даёт ровно единицу, а
    -- единица от целого числа целая, и округлять там нечего. По
    -- библиотеке — ни одного заклинания, которому пять очков не дают
    -- ничего.
    --
    -- ЦЕНА, И ЕЁ НАДО ЗНАТЬ: первое очко теперь не даёт ничего двум
    -- третям библиотеки (203 заклинания из 320 — это сроки в один,
    -- два, три и четыре хода). Навык стал про вложение, а не про
    -- прикосновение.
...
--- @param targetName string|nil  кому эффект достанется. Нужен ТОЛЬКО
---        для вредных: «Внушение» — про чужую волю, и на своих оно не
---        работает (см. врезку ниже). nil означает «получатель
---        неизвестен» — тогда решает один род эффекта, как раньше.
function SB.Logic.EncouragementFor(effectID, targetName)
    ...
    -- другого: помеченное вредом нельзя снять с себя досрочно (см.
    -- рассеивание — с друга снимают дебаффы, с чужого баффы), и этим
    -- закрывают злоупотребление «повесил, снял, повесил заново».
    --
    -- «Последний рубеж» воина и «Небесный промысел» паладина — ровно
    -- такие: по механике это защита, по пометке — вред. И «Внушение»
    -- честно продлевало их владельцу, то есть навык влияния на ЧУЖУЮ
    -- волю удлинял собственный кулдаун.
    --
    -- Поэтому у вредного эффекта спрашиваем ещё и КОМУ он достанется.
    -- Свой (и сам себе — см. SB.Data.IsFriend) — прибавки нет вовсе: ни
    -- «Внушения», которое про чужих, ни «Воодушевления», потому что
    -- эффект сам объявил себя вредом. Тот же список «друзей», по
    -- которому уже разбирается рассеивание, площадь и двойное
    -- заклинание, — правило в аддоне одно на все случаи.
    -- ============================================================
    if targetName and SB.Data.IsFriend and SB.Data.IsFriend(targetName) then
        return 0
    end
...
function SB.Logic.ApplyEffect(effectID, sourceSpell, slotLevel, fromOther, extraTurns, source)
    ...
    -- ЭТА ФУНКЦИЯ ВЕШАЕТ ЭФФЕКТ НА СЕБЯ — всегда, на всех путях: она
    -- кладёт его в SB.ActiveEffects, а это список локального игрока.
    -- Значит получатель известен точно, и спрашивать навык вслепую
    -- незачем: вредный эффект на себе прибавки не получает
    -- (см. врезку «ВНУШЕНИЕ НЕ РАБОТАЕТ НА СВОИХ» у EncouragementFor).
    --
    -- Ровно этим путём приходит «Последний рубеж»: контейнер на
    -- заклинателя, помеченный вредом, чтобы его нельзя было снять с
    -- себя досрочно.
    if extraTurns == nil then
...
-- нажатием и тем же кодом, что любая другая вещь из сумки.
--
...
function SB.Logic.HandlePvpAttackReceived(attackerName, spellID, atkRoll, atkMod, atkTotal, atkCrit,
    ...
    -- КРИТУ ЗАЩИТА НЕ БРОСАЕТСЯ ВОВСЕ.
    --
    -- Крит попадает всегда (см. landed ниже), так что попадание бросок
    -- уже не решает, а больше применить его было некуда — и всё же куб
    -- катился и печатался в строку боя числом, которое ни на что не
    -- влияет. Ровно с этой жалобы правило и начали: «крит, а страж всё
    -- равно кидает куб».
    --
    -- Оговорка «кроме заклинаний с дебаффом» здесь была ровно до тех
    -- пор, пока итогом защиты мерялось ЗАКРЕПЛЕНИЕ чар: отбить крит
    -- нельзя, но второй проверке итог был нужен. Второй проверки больше
    -- нет (см. врезку о дебаффе ниже), и оговорка ушла вместе с ней.
    local skipDefense = guaranteed or atkCrit
    ...
    --
    -- ПОПАЛ — ЗНАЧИТ ЗАЦЕПИЛОСЬ. Второй проверки у дебаффа больше нет.
    --
    -- Здесь стоял отдельный бросок на закрепление: удар проходил, а
    -- чары могли и не зацепиться — против них считалась своя планка из
    -- атрибута, названного в карточке дебаффа, плюс «Внушение»
    -- нападающего. Два исхода на один бросок, и игроку приходилось
    -- читать в строке боя обе развязки подряд.
    --
    -- ОДНО ДЕЙСТВИЕ — ОДИН ИСХОД. У уронного и лечащего заклинания
    -- бросок уже был, и он уже ответил на единственный вопрос, который
    -- тут есть: достал ли ты цель. Отбили удар — нет и чар; прошёл —
    -- пришли вместе с ним. Это не только короче, но и честнее: раньше
    -- вложенная стойкость работала ДВАЖДЫ — один раз в защите, второй в
    -- скрытой планке поверх неё.
    --
    -- ЧТО ОСТАЛОСЬ У ЗАЩИЩАЮЩЕГОСЯ. Во-первых, сама защита: не дал
    -- попасть — не получил ничего. Во-вторых, «Воля»: наложенный
    -- контроль снимается срывом, и это активный ответ вместо прежнего
    -- пассивного броска (см. SB.ActiveEffects.ShakeOff). Атрибут из
    -- карточки дебаффа по-прежнему работает там, где бросок И ЕСТЬ
    -- проверка закрепления, — у заклинаний без урона (ResolveEffectCast).
    --
    -- «ВНУШЕНИЕ» СЮДА БОЛЬШЕ НЕ ЕДЕТ: навык продлевает срок, а не
    -- поднимает шанс зацепиться (см. врезку о нём в Core/Skills.lua).
    local debuffLanded = false
    if spell and spell.debuff and landed then
        -- ИМЯ НАПАДАЮЩЕГО ЕДЕТ В ЭФФЕКТ: провокации нужен тот, кто её
    ...
    -- ЗДЕСЬ БЫЛИ ПРИПИСКИ «дебафф наложен» И «дебафф отведён».
    --
    -- Они были нужны, пока у дебаффа был свой бросок: из строки нельзя
    -- было понять, зацепилось ли, и «Воля» без приписки оставалась
    -- невидимой. Второго броска больше нет (см. врезку выше), а значит
    -- и рассказывать нечего: попал — чары легли, отбили — не легли, и
    -- это следует из самого исхода удара, который в строке уже есть.
    --
    -- Что именно легло, видно на панели эффектов и в карточке
    -- заклинания, ссылка на которую стоит в этой же строке.
