...
function SB.Logic.ExecuteForcedOutcome(spellID, outcomeIndex, slotLevel)
    ...
    --
    -- КОНТЕЙНЕР ВЕШАЕМ ДО ХОДА, а не после, как было: только так «что
    -- легло» и «что не тикать» приходят из одного места и не могут
    -- разойтись (см. SB.Logic.ApplyOwnContainer). Порядок для игрока
    -- ничего не меняет — оба действия внутри одного нажатия.
    --
