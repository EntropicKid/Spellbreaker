...
local function ParseREST(sender, t)
    ...
    if t.restType == "LONG" then
        SB.Logic.LocalRest()
        -- СВОЕЙ СТРОКИ НЕ ПЕЧАТАЕМ: Ведущий объявляет отдых строкой в лог
        -- («… объявляет Долгий Отдых. Ресурсы и здоровье восстановлены у
        -- всех!»), и она приходит всем. Вторая, своя, дублировала её.
    end
end
