...
--
-- ЧЕМ ТОГДА ОТЛИЧАЕТСЯ. Ровно двумя вещами, и обе помечены полями:
--   isItem     — предмет, а не заклинание: своя колонка, свой ряд
--                подготовки, своя вкладка библиотеки;
--   consumable — расходуется применением.
--
-- Было и третье — profession, «чьё ремесло его делает». Ремесло было
-- одно, вместе с ним вырезаны все встроенные зелья, и делить предметы
-- стало не на что (см. врезку ниже).
--
...
-- ЗДЕСЬ БЫЛИ РЕМЁСЛА
--
-- Ремесло было одно — алхимия, — и делило предметы на разделы, которых
-- больше нет: встроенные зелья вырезаны целиком, а у сотворённого и у
-- самодельного ремесла нет и не было. Делить стало не на что, и
-- вкладка «Предметы» показывает просто ВСЕ предметы.
--
-- САМА КОЛОНКА ОСТАЛАСЬ, и это главное: через неё работают классовые
-- вещи — «Самоцвет маны», «Камень здоровья», «Чарокамень» и прочее
-- сотворённое (см. Spells/Conjured.lua), а также предметы, собранные
-- игроками в окне создания. Ремесло к ним отношения не имело никогда.
-- ============================================================
...
-- расходуются по одной. После того как встроенная алхимия вырезана,
-- сотворённое и самодельное — единственное, что вообще лежит в сумке.
--
...
--- Всё, что можно взять с собой, по алфавиту.
...
function SB.Items.ListPackable()
    local out = {}
    for _, sp in pairs(SB.Data.Spells or {}) do
        ...
        -- наливают (см. врезку о сотворённых выше): она появляется
        -- кастом и приходит через SB.Items.Grant.
        if SB.Items.IsItem(sp) and not SB.Items.IsConjured(sp) then
            out[#out + 1] = sp
        end
    end
...
-- впятеро, до 75% на полностью вложенном. Механики больше нет: расход
-- предмета снова ровно один за применение, и списывает его NoteUsed
-- ниже, ничего не бросая.
--
-- САМ НАВЫК ОСТАЛСЯ. «Наука» — источник скейлинга у тридцати трёх
-- заклинаний (тридцать из них у Мага), и вынуть её из характеристик
-- значило бы выбить у класса опору, а не убрать одну механику.
-- Своей механики у неё теперь нет — как у «Искусности» и как было у
-- «Воодушевления» до его правки.
-- ============================================================
