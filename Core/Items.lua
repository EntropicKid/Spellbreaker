...
-- носить склянки умеет кто угодно, ранг тут ни при чём. Сколько их —
-- решает «Ремесло» (см. врезку о ячейках ниже).
--
...
--
-- ОДНА базовая — одинаково на всех рангах (см. врезку выше). Сверх неё
-- ячейки открывает «РЕМЕСЛО», по одной на 3 и 5 очков: до трёх.
--
...
-- навыка.
--
-- БЫЛО ТРИ БАЗОВЫХ И ПОРОГИ 2/3/5, то есть до шести ячеек. Сумка на
-- шесть пачек означала, что склянки перестали быть выбором: с собой
-- бралось всё нужное сразу, и «что взять» не спрашивалось вовсе. Теперь
-- невложенное «Ремесло» даёт ровно одну ячейку — одну пачку на сцену, —
-- а каждая следующая стоит трёх и пяти очков навыка соответственно.
--
-- ПОРОГИ НЕРОВНЫЕ (3 и 5), и это не опечатка: вторая ячейка достаётся
-- на середине навыка, третья — только на полностью вложенном. Тот, кто
-- носит три пачки, заплатил за это всем «Ремеслом».
--
...
-- от зелья и исчезать от проклятия. Игрок, разложивший три склянки, не
-- обязан гадать, куда денется третья, когда на него навесят дебафф.
-- ============================================================
SB.Items.BASE_PREPARED = 1

-- Пороги «Ремесла», по одной ячейке за каждый достигнутый.
SB.Items.SLOT_STEPS = { 3, 5 }
...
---
--- Не Unprepare: тот отказывает при замке набора, а замок стережёт «взял
--- с собой» — состав сумки не меняют после первого каста. Кража же
--- происходит ровно посреди сцены и является чужим действием, а не
--- перекладыванием своих вещей. Та же развилка, по которой сотворение
...
-- когда «Ремесло» их открывало, и сами оттуда не уходят. Сбрось навык
-- обратно — ячеек станет меньше, а пачки останутся и будут пить всю
-- сцену. Правило превращается в «нужно Ремесло НА МОМЕНТ сбора», то
-- есть в ничто: вложил, разложил три пачки, вернул очки в другой навык.
--
...
--
-- ЗАМОК НАБОРА ЗДЕСЬ НЕ ДЕЙСТВУЕТ, в отличие от SB.Items.Unprepare.
...
-- «Ремесло» было выше, а SKILLS_CHANGED на входе никто не шлёт — навык
-- не менялся, он просто загрузился.
