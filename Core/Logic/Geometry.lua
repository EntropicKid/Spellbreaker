...
--
-- ДВА МЕТРА — по итогам сцен: метра за очко не хватало, скрытность
-- почти не ощущалась против дальнобойщиков. Два — середина: Скрытность 7
-- уводит цель на 14 м, из тридцатиметровой дальности остаётся 16, то
-- есть дальний бой по-прежнему дальний. Ближнего боя это не касается —
-- там упор в MELEE_RANGE (см. IsSpellInRange).
local STEALTH_PER_POINT = 2
