...
function SB.Logic.ResolveNpcAttack(spellID, slotLevel)
    ...
    -- ── ИСХОД АТАКИ УХОДИТ В ОБЩУЮ ВОРОНКУ ────────────────
    --
    -- Пока событие звалось PVP_HIT_RESOLVED, этой строки здесь не было,
    -- и «удар по существу» не считался ударом ВООБЩЕ: молчали поводы
    -- onAction("hit"), не спадали эффекты с breakOn.dealt, не копился
    -- классовый ресурс Воина и Охотника на демонов. То есть не работало
    -- ровно там, где идёт основная игра, — в бою с существами.
    --
    -- ПОСЛЕ SpendTurn, а не до: повод умеет вешать эффект на себя
    -- (onAction.effect), и до списания хода он тем же ходом и убыл бы.
    -- В ПвП порядок тот же сам собой — там ответ цели приходит по сети
    -- отдельным кадром, много позже списания.
    --
    -- ИМЕНИ ЦЕЛИ НЕТ НАМЕРЕННО (четвёртый довод). Оно нужно только
    -- возмездию наоборот — onAction.toTarget, — а доставляется эффект
    -- пакетом чужому КЛИЕНТУ. У волка клиента нет, и передать сюда
    -- npcName значило бы отправить бафф игроку с таким же именем.
    SB.Events.Fire(SB.E.ATTACK_RESOLVED, dmg, spellID, landed, nil)
