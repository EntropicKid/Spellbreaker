...
SB.Data.Tooltips["resource_caster"] = {
    title = "Мана",
    -- ТРИ СТРОКИ ВМЕСТО ШЕСТИ. Было по строке на каждое следствие:
    -- плата за круг, усиление характеристик, «на попадание не влияет»,
    -- растяжение эффектов, максимум по рангу, отдых. Всё правда — и всё
    -- вместе читается как параграф, который никто не дочитывает.
    -- Оставлено по строке на вопрос, который человек и задаёт, глядя на
    -- полоску: за что платят, что даёт лишняя единица, откуда берётся.
    lines = {
        "Плата за круг заклинания: сколько влил — в такой круг оно и ушло.",
        function()
            return string.format(
                "Влитое сверх круга усиливает вклад характеристик (+%d%% за единицу) и растягивает эффекты.",
...
SB.Data.Tooltips["resource_Разбойник"] = {
    ...
    lines = {
        ClassResourceLine,
        RegenLine("Копится за СВОИ приёмы — заговоры школы Разбойника, чужие не в счёт.",
                  "Энергии за приём"),
        "Иначе восстанавливается только на отдыхе.",
    },
}
SB.Data.Tooltips["resource_Охотник"] = {
    ...
    lines = {
        ClassResourceLine,
        -- Не через RegenLine: та говорит «за что», а здесь смысл ровно в
        -- том, что ни за что — Фокус набирается сам.
        --
        -- РАСТЁТ ЧАСТОТА, А НЕ ВЕЛИЧИНА, и подпись обязана показывать
        -- именно её: у остальных некастеров по рангу растёт размер
        -- прибавки, и общая формулировка «столько-то по рангу» здесь
        -- была бы неправдой в обе стороны — Фокус всегда идёт по
        -- единице, но всё чаще.
        --
        -- СЕКУНДЫ СЧИТАЮТСЯ ИЗ ХОДОВ, а не пишутся вторым числом: в
        -- свободном ходу ход и есть шесть секунд (RealtimeTick), и
        -- расхождение между «раз в 2 хода» и подписанными вручную
        -- секундами всплыло бы при первой же правке лестницы.
        function()
            local ladder = SB.Data.Config.FocusEveryTurns or {}
            local names, turns, secs = {}, {}, {}
            for _, rank in ipairs(SB.Data.GetMasteryList()) do
                local n = tonumber(ladder[rank]) or 1
                names[#names + 1] = rank
                turns[#turns + 1] = tostring(n)
                secs[#secs  + 1] = tostring(n * 6)
            end
            return string.format(
                "Набирается сам: по 1 Фокусу раз в %s хода по рангу (%s) — " ..
                "в свободном ходу раз в %s сек.",
                table.concat(turns, "/"), table.concat(names, "/"),
                table.concat(secs, "/"))
        end,
        "От действий не зависит — идёт и в бездействии. Иначе только отдых.",
    },
}
SB.Data.Tooltips["resource_Охотник на демонов"] = {
    ...
    lines = {
        ClassResourceLine,
        RegenLine("Копится с любого проваленного броска.", "Ярости"),
        "Иначе восстанавливается только на отдыхе.",
    },
}
