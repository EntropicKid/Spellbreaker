...
AddEffect({
    ...
    -- damageFire, а не damage: бес ЖЖЁТ — это написано и в его описании,
    -- и в том, чем он занят. Общий канал давал прибавку любому удару,
    -- включая те, к которым огонь отношения не имеет. Ровно та же правка,
    -- что у Демона Бездны с его тьмой.
    effect = { family = "Демон", kind = "buff",
               mods = { defense = -12, damageFire = 1, range = 9 },
...
AddEffect({
    ...
    -- damageShadow, а не damage: туша из Пустоты усиливает ТЬМУ, а не
    -- взмах кинжалом. Общий канал дал бы чернокнижнику прибавку к любому
    -- удару, включая те, к которым Бездна отношения не имеет.
    --
    -- ОБЩИЙ ШТРАФ damage = -1 ПРИШЛОСЬ СНЯТЬ. Держать общий канал вместе
    -- со школьным нельзя — они складываются оба, и прибавка считается
    -- дважды (проверка на это стоит в Tests/run.lua и поймала ровно этот
    -- случай). Цена призыва осталась там, где она и читалась: range = -3,
    -- туша загораживает хозяину линию.
    effect = { family = "Демон", kind = "buff",
               mods = { defense = 18, range = -3,
                        damageShadow = 1, resistShadow = 1 },
               stats = { ["Живучесть"] = 2, ["Ношение брони"] = 1, ["Воля"] = 1 } },
})
...
AddEffect({
    ...
    -- resistMagic, а не одна школа: гончая жрёт ЛЮБЫЕ чары — в этом вся
    -- её повадка. «Атлетика» — плата за то, что её приходится
    -- догонять: пёс не ходит шагом.
    effect = { family = "Демон", kind = "buff",
               mods = { armor = 10, resistMagic = 1 },
               stats = { ["Выживание"] = 2, ["Воля"] = 1, ["Атлетика"] = 1 },
               tick = { castResource = -1 } },
})
