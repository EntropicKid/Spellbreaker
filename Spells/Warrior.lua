...
Add({
    ...
    isCantrip = false,
    resistable = true,
    -- ЧИНИТ БРОНЮ, А НЕ ВЕШАЕТ СТОЙКУ.
    --
    -- Здесь стоял контейнер: +20 брони на три хода и −3 «Точности» в
    -- нагрузку. То есть заклинание, которое по описанию про «принять
    -- удар на щит», выдавало временную прибавку и ШТРАФ — а блок щитом
    -- ничем не должен мешать своему носителю.
    --
    -- Починка честнее: доспех в этой системе расходуемый запас на всю
    -- сцену (см. SB.Skills.AbsorbDamage), и «принял удар на щит»
    -- означает ровно то, что запас не просел. Двадцать — тот же размер,
    -- что давала прежняя стойка, только теперь он остаётся.
    repairArmor = 20,
    distance = 0,
	scaling = {
...
Add({
	...
	-- ПОТОК, А НЕ ОДИН УДАР. «Пока связка идёт», «пока поток идёт»,
	-- «пока вращение держится» — все четыре заклинания этого рода
	-- описывают удержание, и все четыре были помечены концентрацией,
	-- которой НЕЧЕГО ДЕРЖАТЬ: ни контейнера, ни баффа, ни держателя.
	-- Флаг стоял и не значил ничего, а заклинание оставалось одиночным
	-- ударом вопреки собственному описанию.
	--
	-- ДВА ПРОДОЛЖЕНИЯ — тариф из библиотеки, тот же, что у «Похищения
	-- души» и «Похищения жизни». Повтор бесплатен по ресурсу, но тратит
	-- ход и требует нового броска, поэтому «бесконечно» он не значит.
	--
	-- Держатель вешается концентрацией всегда (см. ConfirmCast), так что
	-- оговорка «его не остановить ни окриком, ни болью» наконец обретает
	-- обратную силу: контроль поток обрывает.
	channel = 2,    distance = 2.5,
    aoe = { radius = 4.5 },
    scaling = {
